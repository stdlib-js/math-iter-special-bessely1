...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterBessely1, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterBessely1( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the Bessel function of the second kind of order one for each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterBessely1( linspace( 0.0, 2.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 2.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = bessely1( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the Bessel function of the second kind of order one for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': bessely1( values[ 0 ] ),
			'done': false
		},
		{
			'value': bessely1( values[ 1 ] ),
			'done': false
		},
		{
			'value': bessely1( values[ 2 ] ),
			'done': false
		},
		{
			'value': bessely1( values[ 3 ] ),
			'done': true
		}
	];
